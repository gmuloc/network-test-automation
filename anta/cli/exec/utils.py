    ...
    if len(devices) > 0:
        ...
        for device in devices:  # TODO: should use asyncio.gather instead of a loop.
            try:
                ...
                await device.session.cli(command=f"bash timeout 30 zip {EOS_TECH_SUPPORT_ARCHIVE_ZIP} /mnt/flash/schedule/tech-support/*")
            ...
            # Create directories
            tech_support_root_local_path = device_directories_show_tech_support(device.name, root_dir)
