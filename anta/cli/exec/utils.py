...
async def clear_counters_utils(
    ...
    async def clear(dev: InventoryDevice) -> None:
        ...
        # In this case we want to catch all exceptions
            ...
            logger.debug(f"Exception raised for device {dev.name} - {exc_to_str(e)}")
...
async def collect_scheduled_show_tech(
    inv: AntaInventory, root_dir: str, tags: List[str], ssh_port: int = 22
) -> None:
    """
    ...
    if len(devices) > 0:
        ...
        for device in devices:  # TODO: should use asyncio.gather instead of a loop.
            try:
                ...
                logger.error(
                    f"Unable to create tech-support archove on {device.name}: {e.errmsg}"
                )
            ...
            try:
                ...
                with SCPClient(ssh.get_transport()) as scp:
                    scp.get(
                        EOS_TECH_SUPPORT_ARCHIVE_ZIP,
                        local_path=tech_support_device_local_path,
                    )
            ...
            try:
                ...
                await device.session.cli(
                    command=f"bash timeout 30 rm {EOS_TECH_SUPPORT_ARCHIVE_ZIP}"
                )
                logger.info(f"Deleted {EOS_TECH_SUPPORT_ARCHIVE_ZIP} on {device.name}")
            except EapiCommandError as e:
                logger.error(
                    f"Unable to delete tech-support archive on {device.name}: {e.errmsg}"
                )
